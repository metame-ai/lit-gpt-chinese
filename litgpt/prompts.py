...
class Baichuan2(PromptStyle):

    def apply(self, prompt: str, **kwargs: str) -> str:
        history = kwargs.get("history", [])
        res_prompt = ""
        for item in history:
            res_prompt += f"<{item['role']}>{item['content']}"
        res_prompt += f"<user>{prompt}<assistant>"
        return res_prompt


class ChatGLM2(PromptStyle):

    def apply(self, prompt: str, **kwargs: str) -> str:
        history = kwargs.get("history", [])
        is_first = kwargs.get("is_first", True)
        _pair_data = []
        hist_list = []
        for item in history:
...
class ChatGLM3(PromptStyle):
    ...
    def apply(self, prompt: str, **kwargs: str) -> str:
        ...
        is_first = kwargs.get("is_first", True)
        res_prompt = ""
        if len(system_message) and is_first:
            res_prompt += f"<|system|>\n{system_message}"
        for item in history:
            res_prompt += f"<|{item['role']}|>\n{item['content']}"
        res_prompt += f"<|user|>\n{prompt}<|assistant|>\n"
        return res_prompt
...
class Yi(PromptStyle):
    ...
    def apply(self, prompt: str, **kwargs: str) -> str:
        ...
        for item in history:
            res_prompt += f"<|im_start|>{item['role']}\n{item['content']}<|im_end|>\n"
        res_prompt += (
            "<|im_start|>user\n"
            f"{prompt}<|im_end|>\n"
            "<|im_start|>assistant\n"
...
class InternLM2(PromptStyle):
    ...
    def apply(self, prompt: str, **kwargs: str) -> str:
        ...
        for item in history:
            ...
            res_prompt += f"<|im_start|>{role}\n{content}<|im_end|>\n"
        res_prompt += f"<|im_start|>user\n{prompt}<|im_end|>\n"
        res_prompt += f"<|im_start|>assistant\n"
        return res_prompt

    def stop_tokens(self, tokenizer: "Tokenizer") -> Tuple[List[int], ...]:
        return ([tokenizer.token_to_id("<|im_end|>")], [tokenizer.eos_id],)
...
def model_name_to_prompt_style(model_name: str) -> PromptStyle:
    ...
    if re.search(r"(Code)?Gemma.*-it", model_name):
        return Gemma()
    if re.search("chatglm2", model_name):
        return ChatGLM2()
    if re.search("chatglm3", model_name):
        return ChatGLM3()
    if re.search(r"yi-.*b-chat", model_name.lower()):
        return Yi()
    if re.search(r"Qwen1.5", model_name):
        return Qwen1_5()
    if re.search("internlm2", model_name):
        return InternLM2()
    if re.search("baichuan2", model_name):
        return Baichuan2()
    return Default()
