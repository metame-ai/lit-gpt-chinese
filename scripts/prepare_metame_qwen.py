...
DATA_FILE_URL = ""
DATA_FILE_NAME = "out_chat_right_20-15.json"
DESTINATION_PATH = Path("data/metame_qwen")
CHECKPOINT_DIR = Path("checkpoints/qwen/Qwen1.5-7B-Chat")
TEST_SPLIT_FRACTION = 0.01  # to get exactly 50 test samples
IGNORE_INDEX = -1
MASK_INPUTS = False  # as in alpaca-lora
SEED = 42
SYSTEM_MESSAGE = "你不是AI助手，你是刚毕业的学生，是我的好朋友，你善于倾听，为人和善，擅长与人聊天，帮助朋友解决生活困扰。你的名字叫小满。"
...
class SupervisedDataset():
    ...
    def preprocessing(self, example):
        ...
        if len(self.system_message):
            system_prompt = f"<|im_start|>system\n{self.system_message}<|im_end|>\n"
            new_tokens = self.tokenizer.encode(system_prompt, return_tensor=False)
            input_ids += new_tokens
            labels += [self.ignore_index] * (len(new_tokens))

        ast_st_tokens = self.tokenizer.encode("<|im_start|>assistant\n", return_tensor=False)
        end_tokens = self.tokenizer.encode("<|im_end|>\n", return_tensor=False)
        ...
        for message in example["conversations"]:
            ...
            if from_ == "human":
                ...
                new_tokens = self.tokenizer.encode(_text, return_tensor=False)
                input_ids += new_tokens
                labels += [self.ignore_index] * (len(new_tokens))
            else:
                input_ids += ast_st_tokens + value_ids + end_tokens
                labels += [self.ignore_index] * len(ast_st_tokens) + value_ids + end_tokens
